let
  ...
  nixpkgsSrc = if haskellNix.pkgs.lib.strings.hasSuffix builtins.currentSystem "darwin" then sources.nixpkgs-darwin else haskellNix.sources.nixpkgs-2009;
...
rec {
  ...
  postgrest = (pkgs.haskell-nix.hackage-package {
      ...
      #configureFlags = [ "--ghc-option=-optl=-L${pkgs.libpq}/lib" ];
